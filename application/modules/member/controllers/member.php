...
class Member extends MX_Controller {
	...
	function signup()
	{
		...
		$this->form_validation->set_message('valid_email', __('The email address you entered is not valid.', $this->template['module']));			
	...
	function _verify_mail($data)
	{
		...
		{
			$this->form_validation->set_message('_verify_mail', __("The email is already in use", $this->template['module']));
			return FALSE;
		}
	...
	function adino($code = null)
	{
		if (is_null($code))
		{
			...
			$this->load->library('form_validation');
			$this->form_validation->CI =& $this;
			$this->form_validation->set_rules('email',__('Email', $this->template['module']), "trim|required|valid_email|callback__email_not_found");	
			
			$this->form_validation->set_error_delimiters('<p style="color:#900">', '</p>');
			
			$this->form_validation->set_message('required', __('The %s field is required', $this->template['module']));
			$this->form_validation->set_message('valid_email', __('The address %s is not a valid email', $this->template['module']));
	...
	function change_mail()
	{
		...
		$this->form_validation->CI =& $this;
		$this->form_validation->set_rules('email',__('Email', $this->template['module']), "trim|required|matches[remail]|valid_email|callback__verify_mail");
		$this->form_validation->set_rules('remail',__('Confirm', $this->template['module']), "trim|required");
