...
<form  enctype="multipart/form-data" class="edit" action="<?php  echo site_url('admin/downloads/document/save')?>" method="post" accept-charset="utf-8">
		...
		<ul>
			...
			<li><a href="<?php  echo site_url('admin/downloads/')?>" class="input-submit last"><?php  echo __("Cancel", 'downloads')?></a></li>
		</ul>
		...
		<label for="file_id"><?php  echo __("File", 'downloads')?>:</label>
		<span class="input-text"><?php echo $file['name'] ?></span>
...
<br />
		...
		<?php  $follow = null; foreach($parents as $parent) : ?>
			<option value="<?php  echo $parent['id']?>"> &nbsp;<?php  echo ($parent['level'] > 0) ? "|".str_repeat("__", $parent['level']): ""?> <?php  echo $parent['title'] ?> </option>
		<?php  endforeach; ?>
		<?php endif;?> 
		</select><br />
