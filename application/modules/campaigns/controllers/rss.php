<?php   if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Rss extends MX_Controller {
		...
		function index($lang = null)
		{
			...
			$params['where'] = array('campaigns.lang' => $lang);
			
			if($rows = $this->campaigns->get_list($params))
			{
		...
		function cat ($uri = null, $lang = null)
		{
			...
			if(!is_null($lang))
			{
				$params['where']['campaigns.lang'] = $lang;
			}
			if(is_null($uri))
			{
				$params['where']['cat'] =  0;
				$category = array('title' => __("No category", "campaigns"));
			}
			else
			{
				$cat = $this->campaigns->get_cat(array('uri' => $uri, 'lang' => $lang));
		...
		function tag ($tag = null, $lang = null)
		{
			...
			else
			{
				...
				if( $rows = $this->campaigns->get_campaigns_by_tag($params))
				{
					$data['encoding'] = 'utf-8';
		...
		*/
		...
		{
			$this->_settings = unserialize($this->system->campaigns_settings);
