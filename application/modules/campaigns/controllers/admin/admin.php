<?php   if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Admin extends MX_Controller {
		...
		function index($start = 0)
		{
			...
			$data = $this->campaigns->get_list($params);
			...
			$config['base_url'] = site_url('admin/campaigns/index');
			$config['total_rows'] = $this->campaigns->get_total();
		...
		function save()
		{
			...
			$id = $this->campaigns->save();
			$this->plugin->do_action('campaigns_save', $id);
			...
			$this->session->set_flashdata('notification', __("Campaigns saved", $this->template['module']));
		...
		function move($direction, $id)
		{
			...
			$this->campaigns->move($direction, $id);
		...
		function create($id = null)
		{
			...
			$this->user->check_level('campaigns', LEVEL_ADD);
			...
			if (!is_null($id))
			{
				$this->user->check_level('campaigns', LEVEL_EDIT);
			
				$query = $this->db->get_where('campaigns', array('id' => $id));
				$row = $query->row_array();
				$row['tags'] =  $this->campaigns->get_tags(array('where' => array('campaigns_id' => $id),'limit' => 50, 'order_by' => 'cnt DESC' ));
			}

			$this->template['row'] = $row;

			$this->template['categories'] = $this->campaigns->get_catlist();

			$this->template['tags'] = $this->campaigns->get_tags(array('limit' => 50, 'order_by' => 'cnt DESC' ));

			$this->javascripts->add('ajaxfileupload.js');
			
			$this->db->where('src_id', 0);
			$this->db->or_where('src_id', $id);
			$this->db->where('module', 'campaigns');			
		...
		function delete($id, $js = 0)
		{
			...
			if ( $js > 0 )
			{
				...
				$this->session->set_flashdata('notification', 'Campaigns has been deleted.');
				$this->cache->remove('campaigns'.$this->user->lang, 'campaigns'); 
				$this->plugin->do_action('campaigns_delete', $id);
				redirect('admin/campaigns');
			}
			else
			{
			...
			else 
			{
				...
				else
				{
					...
					{						
						...
						$this->image_lib->resize();
						$data = array('file' => $image_data['file_name'], 'module' => 'campaigns');
						$this->db->insert('images', $data);
